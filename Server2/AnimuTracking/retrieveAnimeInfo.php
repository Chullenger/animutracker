 ...
 */
  ...
  $sql = "SELECT m.idanime, m.nombre, m.calif, m.sinopsis,m.fecha_est, m.episodios, m.temporada, m.rating,e.nombre nombre_anima, a.nombre nombre_aut, a.apellido apellido_aut ,l.evaluacion ,l.estado, l.capitulos FROM anime m, autor a,ve l,animadora e, usuario us WHERE m.idautor = a.idautor && m.idanimadora = e.idanimadora && l.idusuario={$iduser} && us.idusuario=l.idusuario && l.idanime = m.idanime && m.idanime={$idanime}";
  ...
  if($result = mysqli_query($con,$sql))
  {
    ...
    while($row = mysqli_fetch_assoc($result)) 
    {
      ...
      $animeDatas[$i]['eval']=$row['evaluacion'];
        $animeDatas[$i]['estado']=$row['estado'];
        $animeDatas[$i]['capitulos']=$row['capitulos'];
      $genres = "";
      $i++;
    }
